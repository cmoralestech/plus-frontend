...
import { getPostBySlug, getAllSlugs, getRelatedPosts } from "@/lib/blog-pt";
...
export async function generateMetadata({ params }: { params: Promise<{ slug: string }> }): Promise<Metadata> {
  ...
  return {
    ...
    alternates: {
      canonical: `https://meetyourplus.com/pt/blog/${slug}`,
      languages: {
    ...
    openGraph: {
      title: post.title,
      description: post.metaDescription,
      url: `https://meetyourplus.com/pt/blog/${slug}`,
      type: "article",
      siteName: "Plus",
...
export default async function BlogPostPage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  const jsonLd = {
    ...
    mainEntityOfPage: { "@type": "WebPage", "@id": `https://meetyourplus.com/pt/blog/${slug}` },
      ...
      <article className="px-6 md:px-12 pt-8 md:pt-16 pb-20 max-w-3xl mx-auto">
        <div className="mb-8">
          ...
          <p className="text-xs mt-4" style={{ color: creamMuted }}>
            By <Link href="/pt/about" style={{ color: gold }}>The Plus Team</Link>
          </p>
        </div>
      ...
      {related.length > 0 && (
        <section className="px-6 md:px-12 pb-16 max-w-3xl mx-auto" style={{ borderTop: `1px solid ${line}`, paddingTop: "2rem" }}>
          ...
          <div className="space-y-4">
            {related.map((r) => (
              <Link key={r.slug} href={`/pt/blog/${r.slug}`} className="block group">
